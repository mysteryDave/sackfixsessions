...
import org.sackfix.fix44.{ExecutionReportMessage, HeartbeatMessage, InstrumentComponent}
...
class SfSessionActorSpec extends TestKit(ActorSystem("MySpec")) with ImplicitSender
  ...
  "A SfSessionActor" should {
    ...
    "handle a normal session" in {
      ...
      val sessOutRouter = new SfSessOutEventRouterStub(sessionActor, TestProbe().ref)
      ...
      sessionActor ! TcpSaysSocketIsClosedMsgIn(probe1.testActor)
